...
async function launchHarness(userDataDir: string, hostDisabled = false): Promise<RunningHarnessApp> {
  const app = await electron.launch({
    ...
    env: {
      ...
      GEOCRM_HARNESS_E2E: '1',
      GEOCRM_DISABLE_CODEX_HOST: hostDisabled ? '1' : '0',
    },
  })
  ...
  await page.waitForFunction(() => Boolean(window.geocrm?.harness))
  return { app, page }
}
...
test('persists device-only Harness settings in SQLite across app restarts', async ({}, testInfo) => {
  ...
  await first.page.evaluate(async () => {
    await window.geocrm.harness.setDevicePreferences({
  ...
  const preferences = await second.page.evaluate(() => window.geocrm.harness.getDevicePreferences())
...
test('covers the shared model control, Computer Use, tools, schedules, library, MCP, and history', async ({}, testInfo) => {
  ...
  try {
    await running.page.evaluate(async (workFolder) => {
      ...
      await window.geocrm.harness.setDevicePreferences({ sidebarVisible: true, workFolder })
    }, canvasWorkspace)
    ...
    await running.page.evaluate(async ({ cwd, canvasSessionId }) => {
      await window.geocrm.harness.parkCanvas(cwd, canvasSessionId)
    ...
    await expect.poll(async () =>
      running.page.evaluate(
        async ({ cwd, canvasSessionId }) => window.geocrm.harness.restoreCanvas(cwd, canvasSessionId),
        { cwd: canvasWorkspace, canvasSessionId },
      ),
    ).toBe(true)
