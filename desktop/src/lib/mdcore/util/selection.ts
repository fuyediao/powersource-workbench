...
export const getEditorRange = (aura: IAura) => {
    let range: Range;
    const element = aura[aura.currentMode].element;
    ...
    if (aura[aura.currentMode].range) {
        return aura[aura.currentMode].range;
    }
    element.focus();
...
export const getCursorPosition = (editor: HTMLElement) => {
    ...
    if (range.getClientRects().length === 0) {
        if (range.startContainer.nodeType === 3) {
            // Empty lines may lack br; get position from parent <p>
    ...
    if (!wbrElement.previousElementSibling!) {
        if (wbrElement.previousSibling!) {
            ...
            if (wbrElement.nextSibling!.nodeType === 3) {
                ...
                // <wbr><br> https://github.com/Vanessa219/aura/issues/400
...
export const insertHTML = (html: string, aura: IAura) => {
    // Engine methods add p elements; remove when there is only one p
    ...
    if (tempBlockElement.length === 1 && !tempBlockElement[0].previousSibling! && !tempBlockElement[0].nextSibling! &&
        aura[aura.currentMode].element.children.length > 0 && tempElement.firstElementChild!.tagName === "P") {
    ...
    if (range.toString() !== "") {
        aura[aura.currentMode].preventInput = true;
        document.execCommand("delete", false, "");
    }

    if (pasteElement.firstElementChild! &&
        pasteElement.firstElementChild!.getAttribute("data-block")! === "0") {
        // When pasted content is a block element, insert in the next paragraph
        pasteElement.lastElementChild!.insertAdjacentHTML("beforeend", "<wbr>");
        const blockElement = hasClosestBlock(range.startContainer);
        if (!blockElement) {
            aura[aura.currentMode].element.insertAdjacentHTML("beforeend", pasteElement.innerHTML);
        } else {
