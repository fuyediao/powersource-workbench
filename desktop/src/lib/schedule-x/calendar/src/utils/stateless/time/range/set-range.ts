...
/** Four consecutive days starting at the selected date (Google-style 4-day). */
export const setRangeForFourDays = (config: RangeSetterConfig): DateRange => {
  let date = config.date as Temporal.ZonedDateTime | Temporal.PlainDate
  if (date instanceof Temporal.PlainDate) {
    date = date.toZonedDateTime({
      timeZone: config.calendarConfig.timezone.value,
  ...
  const endDate = addDays(date, 3) as Temporal.ZonedDateTime
  return {
    start: getRangeStartGivenDayBoundaries(config.calendarConfig, date),
    end: getRangeEndGivenDayBoundaries(config.calendarConfig, endDate),
  }
}

/** Full calendar year of the selected date. */
