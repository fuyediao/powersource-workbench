...
use crate::model_prompts::dedicated_instructions_for_slug;
use codex_utils_output_truncation::approx_bytes_for_tokens;
use tracing::warn;

/// Model-agnostic fallback instructions for slugs missing from `models.json`.
pub const BASE_INSTRUCTIONS: &str = include_str!("../../core/generic-system-prompt.md");
const DEFAULT_PERSONALITY_HEADER: &str = "You are a coding agent. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
/// Replace `instructions_template` with the dedicated catalog dump or the generic fallback.
///
/// Skips local personality slugs so their `{{ personality }}` template stays intact.
/// `config.base_instructions` still wins later in [`with_config_overrides`].
pub(crate) fn apply_catalog_prompt(model: &mut ModelInfo) {
    if matches!(
        model.slug.rsplit('/').next().unwrap_or(&model.slug),
        "gpt-5.2-codex" | "exp-codex-personality"
    ) {
        return;
    }
    let text = dedicated_instructions_for_slug(&model.slug).unwrap_or(BASE_INSTRUCTIONS);
    let model_messages = model.model_messages.get_or_insert(ModelMessages {
        persistent_instructions: None,
        tools: None,
        instructions_template: None,
    ...
    model_messages.instructions_template = Some(text.to_string());
    model_messages.instructions_variables = None;
}
...
fn local_model_messages_for_slug(slug: &str) -> ModelMessages {
    match slug {
        ...
        _ => {
            let text = dedicated_instructions_for_slug(slug).unwrap_or(BASE_INSTRUCTIONS);
            ModelMessages {
                persistent_instructions: None,
                tools: None,
                instructions_template: Some(text.to_string()),
                instructions_variables: None,
                approvals: None,
                collaboration_modes: None,
                auto_review: None,
