...
import { getSnapshotNotices, hideNotice } from '@/services/clash/notice-service'
import {
  clearServiceRequest,
  getServiceRequest,
} from '@/services/clash/service-request'
...
vi.mock('@/hooks/clash/use-system-state', () => ({
  useSystemState: () => ({
    runState: { serviceUsable: false },
    isTunModeAvailable: isTunModeAvailable.current,
  }),
}))
vi.mock('@/hooks/clash/use-system-proxy-state', () => ({
  useSystemProxyState: () => ({
    indicator: false,
    toggleSystemProxy: vi.fn(),
  }),
}))
vi.mock('@/hooks/clash/use-service-uninstaller', () => ({
  useServiceUninstaller: () => ({ uninstallServiceAndStartSidecar: vi.fn() }),
}))
vi.mock('@/components/clash/setting/mods/sysproxy-viewer', () => ({
  SysproxyViewer: () => null,
}))
vi.mock('@/components/clash/setting/mods/tun-viewer', () => ({
  TunViewer: () => null,
}))
