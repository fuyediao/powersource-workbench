...
import { useTrafficGraphDataEnhanced } from '@/hooks/clash/use-traffic-monitor'
...
import {
  ...
  formatTrafficName,
} from '@/utils/clash/traffic-sampler'
...
function resolveCssColor(name: string, fallback: string): string {
  ...
  probe.remove()
  return computed.startsWith('rgb') || computed.startsWith('#') ? computed : fallback
}
 ...
 */
function readTrafficGraphColors(): {
  up: string
  down: string
  grid: string
  text: string
  ...
  return {
    down: resolveCssColor('--clash-traffic-down', '#0ea5e9'),
    up: resolveCssColor('--clash-traffic-up', '#10b981'),
    grid: resolveCssColor('--clash-divider', 'rgba(24, 24, 27, 0.08)'),
    text: resolveCssColor('--muted', '#71717a'),
    background: resolveCssColor('--clash-workspace', '#ffffff'),
  }
}

/**
 * Builds an `rgba()` color for canvas gradients. Theme tokens resolve to `rgb()` or hex;
 * appending a hex alpha suffix only works for hex.
 * @param color - CSS color from {@link readTrafficGraphColors}.
 * @param alpha - Opacity in `[0, 1]`.
 * @returns Canvas-parseable `rgba(r, g, b, a)` string.
 */
...
export const EnhancedCanvasTrafficGraph = memo(
  function EnhancedCanvasTrafficGraph({
    ...
    const colors = useMemo(() => readTrafficGraphColors(), [theme])
    ...
    const drawTrafficLine = useCallback(
      (
        ...
        if (withGradient && chartStyle === 'bezier') {
          ...
          gradient.addColorStop(0, colorWithAlpha(color, GRAPH_CONFIG.alpha.gradient))
