/**
 * Customer detail tab chrome: one row when wide enough; otherwise two equal-width rows.
...
/**
 * Renders detail tabs in one row when the host is wide enough; otherwise two rows.
...
export function CustomerDetailTabBar({
   ...
   * @returns Row element.
   */
  function renderGrid(tabs: CustomerDetailTabId[]) {
    return (
      <div
      ...
      >
        {tabs.map((tabId) => {
          ...
          return (
            ...
            >
              <span className="block truncate">{t(DETAIL_TAB_LABEL_KEY[tabId])}</span>
            </button>
          )
        })}
      </div>
  ...
  return (
    <div ref={hostRef} className="relative">
      {twoRows ? (
        <div>
          {renderGrid(DETAIL_TAB_ROW1)}
          {renderGrid(DETAIL_TAB_ROW2)}
        </div>
      ) : (
        renderGrid(DETAIL_TAB_ALL)
      )}
    </div>
  )
}
