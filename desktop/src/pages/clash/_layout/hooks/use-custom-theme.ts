 ...
 */
/**
 * Converts a CSS color (including oklch) into a value MUI `alpha()` can parse.
 * @param value - Computed CSS color.
 * @param fallback - Hex or rgb fallback.
 * @returns rgb() or hex string.
 */
function toMuiColor(value: string, fallback: string): string {
...
/**
 * Reads a CSS custom property from the hosted document as a MUI-safe color.
 * @param name - Variable name including dashes.
 * @param fallback - Fallback color.
 * @returns Computed rgb/hex value or fallback.
 */
function hostedCss(name: string, fallback: string): string {
  if (typeof document === 'undefined') {
    return fallback
  }
  const raw =
    ...
    fallback
  return toMuiColor(raw, fallback)
}
...
export const useCustomTheme = () => {
  ...
  const [, setHostedTick] = useState(0)

  useEffect(() => {
    if (hosted) {
      const apply = () => {
        setMode(hostedIsDark() ? 'dark' : 'light')
        setHostedTick((tick) => tick + 1)
      }
      apply()
  ...
  }, [hosted, theme_mode, setMode])

  useEffect(() => {
    if (hosted || theme_mode !== 'system' || !appWindow) {
      return
    }
  ...
  }, [hosted, theme_mode, appWindow, setMode])

  useEffect(() => {
    if (hosted || !appWindow || theme_mode === undefined) {
      return
    }
  ...
  }, [hosted, mode, appWindow, theme_mode])

  const theme = useMemo(() => {
    const setting = hosted ? {} : theme_setting || {}
    const base = mode === 'light' ? defaultTheme : defaultDarkTheme
    const dt = hosted
      ? {
          ...base,
          primary_color: hostedCss('--brand', base.primary_color),
          info_color: hostedCss('--brand', base.info_color),
          primary_text: hostedCss('--ink', base.primary_text),
          secondary_text: hostedCss('--muted', base.secondary_text),
          background_color: hostedCss('--canvas', base.background_color),
          font_family:
    ...
    return muiTheme
  }, [hosted, mode, theme_setting, userBackgroundImage, hasUserBackground])
