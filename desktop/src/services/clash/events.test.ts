...
const clashListen = vi.hoisted(() => vi.fn())

vi.mock('@/services/clash/bridge', () => ({ clashListen }))

/** A `clashListen` whose registrations only resolve when the test says so. */
const deferredListen = () => {
  const resolvers: Array<() => void> = []
  clashListen.mockImplementation(
    () =>
      new Promise((resolve) => {
...
describe('subscribeVergeEvents', () => {
  ...
  it('still reports subscribed when a listener fails to register', async () => {
    ...
    clashListen.mockRejectedValue(new Error('no such event'))
