...
/** Compact schedule row for the home aside widget. */
export interface HomeScheduleItem {
  ...
  /** True when planned and scheduled_at is in the past. */
  overdue: boolean
}

/** Home schedule widget payload (preview rows + total planned count). */
export interface HomeScheduleResult {
  items: HomeScheduleItem[]
  totalCount: number
}

/** Max rows shown in the home Schedule Reminder widget. */
export const HOME_SCHEDULE_PREVIEW_LIMIT = 3

/**
 * Maps a follow-up row into a home schedule preview item.
 * @param fu - Follow-up.
 * @param now - Epoch ms for overdue check.
 * @returns Preview item.
 */
function toHomeScheduleItem(fu: FollowUp, now: number): HomeScheduleItem {
  const scheduledMs = new Date(fu.scheduledAt).getTime()
  const subtitle =
    (fu.customerName ?? '').trim() ||
    (fu.leadName ?? '').trim() ||
    (fu.opportunityName ?? '').trim() ||
    (fu.kolName ?? '').trim() ||
    (fu.competitorShopName ?? '').trim() ||
    fu.todoItems.find((item) => item.text.trim())?.text.trim() ||
    '—'
  return {
    id: fu.id,
    type: fu.type,
    ...
    opportunityId: fu.opportunityId,
    kolId: fu.kolId,
    competitorShopId: fu.competitorShopId,
    overdue: Number.isFinite(scheduledMs) && scheduledMs < now,
  }
}

/**
 * Loads planned follow-ups for the home schedule widget (includes overdue).
 * Only the soonest `limit` rows are returned; `totalCount` is the full planned set.
 * @param ownerId - Auth user id.
 * @param limit - Max preview rows (default {@link HOME_SCHEDULE_PREVIEW_LIMIT}).
 * @returns Preview items sorted by scheduled_at ascending, plus total count.
 */
export async function listHomeScheduleFollowUps(
  ownerId: string,
  limit = HOME_SCHEDULE_PREVIEW_LIMIT,
  ...
  if (error) {
    // Retry without association embeds when PostgREST cannot resolve joins.
    const plain = await fromLoose('follow_ups')
      .select('*', { count: 'exact' })
      .eq('owner_id', ownerId)
      .eq('status', 'planned')
      .order('scheduled_at', { ascending: true })
      .range(0, previewLimit - 1)
    if (plain.error) {
      console.error('[follow-ups-api] listHomeScheduleFollowUps:', plain.error)
      throw plain.error
    }
    data = plain.data
    count = plain.count
  ...
  const items = (data ?? []).map((row) =>
    toHomeScheduleItem(mapRowToFollowUp(row as Record<string, unknown>), now),
  )
  return {
    items,
