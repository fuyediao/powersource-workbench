/**
 * Folio scope: personal vs group, role flags, and group switcher options.
 */
...
/** Folio workspace scope. */
export type FolioScopeMode = 'personal' | 'group'

/** Capability flags for the active Folio scope. */
export interface FolioCapabilities {
  canCreate: boolean
  canEdit: boolean
  canDelete: boolean
...
export interface FolioScopeState {
  mode: FolioScopeMode
  setMode: (mode: FolioScopeMode) => void
...
/**
 * Loads Folio RBAC + group list for the signed-in user.
 * @param userId - Auth user id.
 * @returns Scope state for Folio chrome.
 */
export function useFolioScope(userId: string | null | undefined): FolioScopeState {
  const [mode, setMode] = useState<FolioScopeMode>('personal')
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const { data: grants } = await supabase
        .from('group_desktop_writes_folio')
        .select('action')
        .eq('group_id', selectedGroupId)
        .eq('user_id', userId)
        .eq('resource_key', 'pages')
  ...
  const capabilities = useMemo((): FolioCapabilities => {
    if (mode === 'personal') {
      return {
        canCreate: true,
        canEdit: true,
