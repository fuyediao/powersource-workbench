...
import { TrafficDataSampler, formatTrafficName } from '@/utils/clash/traffic-sampler'

// Reference-count manager for the shared traffic worker.
class ReferenceCounter {
  ...
  increment(): () => void {
    this.count++
    debugLog(`[ReferenceCounter] increment: ${this.count}`)

    this.notify()

    return () => {
      this.count--
      debugLog(`[ReferenceCounter] decrement: ${this.count}`)
    ...
    if (typeof Worker !== 'undefined') {
      try {
        this.worker = new Worker(
          new URL(
            '../../services/clash/traffic-monitor-worker.ts',
            import.meta.url,
          ),
          { type: 'module' },
        )
...
/**
 * Enhanced traffic monitor: Web Worker sampling and compression.
 */
export const useTrafficMonitorEnhanced = (options?: {
  ...
  // Register reference counting and worker lifecycle.
  useEffect(() => {
    if (!enabled) return
  ...
  // Request a different time-range snapshot.
...
/**
 * Chart data hook wrapping the enhanced traffic monitor.
 */
