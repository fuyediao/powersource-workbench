...
/** User-defined Gmail label (unused for IMAP). */
...
/** Sidebar navigation target (virtual label or IMAP folder). */
...
/** Host/port preset for a provider id. */
...
/** Bytes returned over IPC for an attachment or .eml download. */
export interface MailBinaryDto {
  bytes: ArrayBuffer
  filename: string
  contentType: string
}
