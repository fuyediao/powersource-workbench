...
The ignored `supabase/.env` file is generated by `scripts/configure-local-env.py` from `/opt/supabase-project/.env` on the Workbench VPS. It holds the public URL, publishable key, server key, and the bootstrap username. It never ships in Electron.
